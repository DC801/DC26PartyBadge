...
int SpaceInvaders(void) {
    ...
    // Load sprites into memory for fast access
    ...
    while (Invaders.tank.lives != 0 && !Invaders.quitGame) {
        ...
        // Try make the delay about the same if we moved the tank
        // Doesn't really help a ton, but, eh
        if (buttonPress) {
            nrf_delay_us(8);
        } else {
            nrf_delay_ms(20);
        ...
        if (updateCounter++ >= (20 / (Invaders.level + 1))) {
            updateInvaders(true);
            updateCounter = 0;
        }
...
static void drawTank(uint8_t x) {
    ...
    // Draw the new one
    util_gfx_draw_raw(x, 116, 12, 12, Invaders.tankSprite);
...
static void updateInvaders(bool move) {
    ...
    util_gfx_fill_rect(0, 20, GFX_WIDTH, maxY - 10, COLOR_BLACK);
...
static void drawInvader(uint8_t x, uint8_t y, uint8_t type) {
    util_gfx_draw_raw(x, y, 12, 10, Invaders.invaderSprite[type]);
}
...
static void updateScore(int points) {
    ...
    char header[25];
    snprintf(header, 25, "L%d  %u", Invaders.level, Invaders.score);
    util_gfx_print(header);

    for (uint8_t i = 0; i < Invaders.tank.lives; i++) {
        util_gfx_draw_raw(GFX_WIDTH - ((TANK_WIDTH + 2) * (i + 1)), 0, TANK_WIDTH, TANK_HEIGHT, Invaders.tankSprite);
    }
...
static void updateBullet(void) {
    ...
    if (Invaders.bullet.active) {
        ...
        util_gfx_fill_rect(Invaders.bullet.x - 2, Invaders.bullet.y - 2, 4, 4, COLOR_BLACK);
        Invaders.bullet.y -= 4;
        util_gfx_fill_rect(Invaders.bullet.x - 2, Invaders.bullet.y - 2, 4, 4, COLOR_WHITE);
